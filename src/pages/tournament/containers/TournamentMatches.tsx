...
const TournamentMatches = ({ details = [] }: TournamentMatchesProps) => {
  ...
  const [filteredDetails, setFilteredDetails] =
    useState<TournamentMatchDetail[]>(details);
  const [filter, setFilter] = useState<string>('All');

  useEffect(() => {
    const fetchUsers = async () => {
      const userIds = details
        .map((detail) => [
          detail.playerId1,
          detail.playerId2,
          detail.playerId3,
          ...
          <Col span={24} key={detail.id}>
            <Card bordered={false}>
              ...
              <Text strong>
                ...
                <Tag color={getResultTagColor(detail.result)}>
                  {detail.result}
                </Tag>
              </Text>
              <br />
...
const MemoizedTournamentMatches = React.memo(TournamentMatches);
export default MemoizedTournamentMatches;
