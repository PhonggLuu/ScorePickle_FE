        ...
        >
          <div>
            ...
            {record.user?.dateOfBirth
              ? new Date(record.user.dateOfBirth).toLocaleDateString()
              : 'N/A'}
          </div>
          <div style={{ marginTop: 4 }}>
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ padding: '6px 0' }}>
          <Space>
            {record.isAccept ? (
              <Button danger size="small" onClick={() => handleBan(record)}>
                Ban
              </Button>
            ) : (
              <Button
    ...
    return (
      <div style={{ padding: 24, display: 'flex', justifyContent: 'center' }}>
        <Typography.Title level={4} type="danger">
          Error loading referees data
        </Typography.Title>
      </div>
    );
  }

  const activeReferees =
    referees?.filter((ref) => ref.user.status === true)?.length || 0;
    ...
    <div style={{ padding: '24px', backgroundColor: '#f0f2f5' }}>
        ...
        <Col span={8}>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              >
                <UserOutlined
                  style={{ fontSize: 32, color: '#52c41a', marginRight: 16 }}
                />
                <Text
        ...
        <Col span={8}>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              >
                <UserOutlined
                  style={{ fontSize: 32, color: '#f5222d', marginRight: 16 }}
                />
                <Text
                  style={{ fontSize: 32, fontWeight: 'bold', color: '#f5222d' }}
                >
                  {inactiveReferees}
                </Text>
              </div>
      ...
      >
        <Space>
          <Button
            type="primary"
            onClick={() =>
              queryClient.invalidateQueries({ queryKey: ['GET_ALL_REFEREES'] })
            }
            icon={<SearchOutlined />}
          >
            Refresh Data
      ...
      >
        <Form
          ...
          layout="vertical"
          onFinish={onFinish}
          initialValues={{ refereeCode: user?.id }}
        >
