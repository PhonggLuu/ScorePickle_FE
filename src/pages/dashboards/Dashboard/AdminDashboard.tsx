...
import { useGetTournaments } from '@src/modules/Tournament/hooks/useGetAllTournaments';
import { useGetAllUser } from '@src/modules/User/hooks/useGetAllUser';
...
const AdminDashboard: React.FC<AdminDashboardProps> = () => {
  ...
  const statistics = useMemo(() => {
    ...
    return {
      ...
      scheduledTournaments:
        tournamentsData && Array.isArray(tournamentsData)
          ? tournamentsData.filter((t) => t.status === 'Scheduled').length
          : 0,
      ongoingTournaments:
        tournamentsData && Array.isArray(tournamentsData)
      ...
      category:
        blogCategoriesData && Array.isArray(blogCategoriesData)
          ? blogCategoriesData.length
          : 0,
      rule: rulesData && Array.isArray(rulesData) ? rulesData.length : 0,
    };
  }, [
    tournamentsData,
    usersData,
  ...
  const tournamentStatusData = useMemo(
    () => [
      ...
      {
        status: 'Scheduled',
        value: statistics.scheduledTournaments,
        color: '#1810df',
      },
    ],
    [statistics]
  ...
  const userRoleData = useMemo(() => {
    ...
    return Object.entries(roleCounts)
      .filter(([, value]) => value > 0) // Only include roles that have users
  ...
  const renderTournamentStatusChart = () => {
    ...
    const config = {
      ...
      radius: 0.8,
      innerRadius: 0.6,
      color: ['#52c41a', '#faad14', '#1890ff', '#1810df'],
      label: {
        type: 'inner',
        offset: '-50%',
        content: '{value}',
          ...
          >
            ...
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              >
                ...
                <Badge
                  status="error"
                  style={{ color: '#1810df !important' }}
                  text={
                    <Text strong>
                      {statistics.scheduledTournaments} Scheduled
                    </Text>
                  }
                />
              </div>
              ...
              />
              <Link to="/admin/tournament/overview">
                <Button
                  type="primary"
                  style={{
                    marginTop: '12px',
