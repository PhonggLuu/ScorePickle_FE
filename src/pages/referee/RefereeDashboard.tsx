...
import { useGetTournamentByRefereeId } from '@src/modules/Referee/hooks/useGetTournamentByRefereeId';

type DataIndex = string;

export const RefereeDashboard = () => {
  const user = useSelector((state: RootState) => state.auth.user);
  const { data, isLoading, refetch } = useGetTournamentByRefereeId(
    user?.id ?? 0
  );
    ...
    },
    {
      ...
      render: (isAccept: boolean) => (
        <div style={{ padding: '6px 0' }}>
          <Tag
            color={isAccept ? 'success' : 'processing'}
            style={{ margin: 0 }}
          >
      ...
      render: (_, record) => (
        <div style={{ padding: '6px 0' }}>
          <Button type="link" style={{ padding: '4px 0' }}>
            <Link to={`/referee/${record.id}`}>Details</Link>
          </Button>
        </div>
      ),
      width: 100,
